...
// use std::{collections::btree_map::Values, convert::Infallible, fmt::Error, num::ParseIntError, path::{self, Path}, process, str::FromStr};
...
//         match s {
//             _ if let Ok(parsed) = s.parse::<isize>()=> {
//                 // parses as isize
//                 Ok(Bool(bool))
//             },
//             _ if let Ok(parsed) = s.parse::<isize>() => {
...
//                 Ok(Signed(parsed))
//             },
//             _ if let Ok(parsed) = s.parse::<usize>() => {
//                 // parses as usize
//                 Ok(Int(parsed))
//             },
//             _ if let Ok(parsed) = s.parse::<Path>() => {
//                 Ok(FilePath(parsed))
//             }
